...
function FindExerciseByNumber(vezbaNumber,vezbicki) {
  values = Object.values(vezbicki);
  if (vezbaNumber === "" || vezbaNumber <= 0) {
    alert(`Please input a valid number greater than 0 and less than or equal to ${values.length}`);
  }  else if (vezbaNumber > values.length) {

    alert("too much excercises, give the students some time to learn!!");

  }else if (vezbaNumber <= values.length) {
    container.innerHTML += `<p> Vezba ${vezbaNumber}: ${values[vezbaNumber - 1]}</p>`;
    container.innerHTML += `---------------------------------------------------`;
  } else {
    alert(`Please input a valid number less than or equal to ${values.length}`);
  }
}
...
function PrintExcercises(inputObject, numOfProps) {
  ...
  if (numOfProps === "" || numOfProps <= 0) {

    alert(`Please input a valid number under or equal to ${values.length}`);

  } else if (numOfProps > values.length) {

    alert("too much excercises, give the students some time to learn!!");

  } else if (numOfProps <= values.length) {

    for (let i = 0; i < numOfProps && i < values.length; i++) {
